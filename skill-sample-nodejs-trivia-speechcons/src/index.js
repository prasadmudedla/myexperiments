...
const languageString = {
    'en': {
        'translation': {
            ...
            CORRECT_SPEECHCON :
            ["Bazinga","Woohoo","mamma mia","bingo","cha ching","hip hip hooray"],
            WRONG_SPEECHCON :
            ["Oh dear","boing","ouch","shucks"]
        },
    },
    'en-US': {
        'translation': {
    ...
    'en-IN': {
        'translation': {
            ...
            // 'CORRECT_SPEECHCON' : "balle balle",
            // 'WRONG_SPEECHCON' : "aiyo"
            CORRECT_SPEECHCON :
            ["balle balle","ji huzoor","shabash","waaaah","cha ching","hip hip hooray"],
            WRONG_SPEECHCON :
            ["dishoom","na-na","rascala","shucks"]
        },
    },
    'en-GB': {
        'translation': {
...
function handleUserGuess(userGaveUp) {
    ...
    if (answerSlotValid && parseInt(this.event.request.intent.slots.Answer.value, 10) === this.attributes['correctAnswerIndex']) {
        currentScore++;
         response = "<say-as interpret-as='interjection'>" + this.t('CORRECT_SPEECHCON')[(Math.floor(Math.random() * this.t('CORRECT_SPEECHCON').length))] + "! </say-as><break strength='strong'/>";
        speechOutputAnalysis =  this.t('ANSWER_CORRECT_MESSAGE');
    } else {
        if (!userGaveUp) {
           response = "<say-as interpret-as='interjection'>" + this.t('WRONG_SPEECHCON')[(Math.floor(Math.random() * this.t('WRONG_SPEECHCON').length))] + "! </say-as><break strength='strong'/>";
...
{
    var speechCon = "";
    if (type) return "<say-as interpret-as='interjection'>" + this.t('CORRECT_SPEECHCON')[(Math.floor(Math.random() * this.t('CORRECT_SPEECHCON').length))] + "! </say-as><break strength='strong'/>";
    else return "<say-as interpret-as='interjection'>" + this.t('WRONG_SPEECHCON')[(Math.floor(Math.random() * this.t('WRONG_SPEECHCON').length))] + " </say-as><break strength='strong'/>";
}
